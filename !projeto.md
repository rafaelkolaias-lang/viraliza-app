...
| `src/lib/gemini-vision.ts`, `llm.ts` | Gemini 2.5 Flash (analisar produto) e LLM próprio (qwen, usado pelo minerador) |
...
| `src/app/api/jobs/**`, `api/cortes/`, `api/leads/buscar/` | pipeline antigo: trava de crédito + limite de jobs simultâneos |
...
**Topo:** `inicio`, `/painel` (Meus vídeos), `shopee`, **`tiktok`** (Produtos TikTok), `acervo`, **`meus-avatares`** (Personalize com IA), **`lab`** (Viraliza Labs), **`viral-boost`**, **`minerador`**, **`academy`** (placeholder "em breve").
**Grupo Ferramentas** (retrátil): `novo` (Editor automático), `lote` (marca em lote), `leads` (MapsLeads), `cortes`.
...
**Outras:** `conta`, `extrato`, `membro`, `produtos`, `virais`, `videos/[id]`, `ferramentas`. `/painel/avatar` é rota morta (redirect pro `lab`).
...
**Viraliza Labs** (`/painel/lab`, aberto a todos os logados). Funil guiado em 3 etapas: **cena** (estilo -> produto -> influenciador -> cenário -> resumo) -> **imagem** -> **vídeo** (config -> movimento -> resumo). Dock com 4 ferramentas: funil, **Minhas imagens** (galeria, dá pra gerar vídeo de uma imagem já paga sem refazer), **Vídeo livre** (chat estilo Grok: prompt livre + imagens + 6/10/15s + idioma) e **Gerador de prompt** (IA com visão escreve a ficha técnica; grátis). UI em `src/components/app/viraliza-lab.tsx` + `lab-*.tsx`.
