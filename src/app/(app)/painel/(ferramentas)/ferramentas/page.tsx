...
/**
 * Porta de entrada do grupo "Ferramentas": apresenta as 7 e diz o que cada uma
...
const FERRAMENTAS: CartaoFerramenta[] = [
  // OS DOIS EDITORES (12/08/2026). Três coisas combinadas aqui:
  //
  // 1. O BASIC vem PRIMEIRO, igual no menu lateral e na barrinha do rodapé: a
  //    ordem das três superfícies é a mesma, senão quem viu o cartão aqui
  //    procura o item no mesmo lugar da barra e não acha.
  // 2. Cada um tem a SUA arte no serverrk (`editor-automatico.jpg` e
  //    `editor-basico.jpg`, subida em 12/08/2026). As duas são a mesma cena, a
  //    varinha em cima do editor, então continuam lendo como a mesma ferramenta.
  // 3. Por serem parecidas, cada um leva o `selo` verde no canto ("BASIC" /
  //    "PRO"): é o que separa os dois de longe, antes da pessoa ler o título.
  //
  // Ver o cabeçalho do `editor-basico.tsx`.
  {
    ...
    descricao:
      "O mesmo editor, na versão simples: tudo numa tela só. Você sobe as mídias, mexe nos ajustes do lado e gera - sem responder o passo a passo.",
  },
  {
    href: "/painel/novo",
    ...
    Icone: Sparkles,
    descricao:
      "Você já gravou o vídeo e ele monta o resto: corta, escreve a copy, põe legenda ou narração e encaixa as fotos do produto no meio da sua fala. Vai em 5 etapas, uma pergunta de cada vez.",
  },
  {
  ...
  return (
    <div>
      <LabCabecalho
        ...
        descricao="São sete aqui dentro, e elas não competem entre si: cada uma resolve um pedaço diferente. Veja o que cada uma faz e escolha por onde começar."
      />
      <div className="mt-8 duration-500 animate-in fade-in slide-in-from-bottom-4">
        {/* 4 colunas: com 7 cartões, 3 colunas deixariam o último sozinho na
            última linha; em 4 a sobra é uma linha de 3. A grade centraliza a
            linha incompleta (é flex, não grid - ver grade-ferramentas.tsx). */}
        <GradeFerramentas itens={FERRAMENTAS} colunas={4} />
      </div>
    </div>
  );
}
