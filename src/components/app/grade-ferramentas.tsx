...
export type CartaoFerramenta = {
  ...
  imagemLocal?: string;
  /** Etiqueta verde no canto de cima da arte (12/08/2026). Existe pros dois
   *  editores automáticos, BASIC e PRO: as artes deles são a mesma cena (a
   *  varinha em cima do editor), então de longe os cartões se confundem. Use só
   *  quando dois cartões da mesma tela se parecem; virar enfeite tira o efeito. */
  selo?: string;
};

/**
 * LARGURA DE CADA CARTÃO, por faixa de tela.
 *
 * É `flex-wrap` e não `grid` de propósito (12/08/2026): a grade alinhava a
 * ÚLTIMA LINHA à esquerda, e com 7 cartões em 4 colunas os 3 que sobravam
 * ficavam encostados num canto, com um buraco do lado. Grid não centraliza linha
 * incompleta - `justify-items` mexe no item dentro da célula, e as células vazias
 * continuam existindo. Com flex + `justify-center` a sobra fica no meio.
 *
 * O preço de trocar grid por flex é ter que dizer a largura na mão: cada linha
 * tem (colunas - 1) espaçamentos de `gap-3` (0,75rem) pra descontar do 100%.
 * Mexeu no `gap` da grade, mexa nestas contas junto.
 */
const LARGURA_CARTAO: Record<3 | 4, string> = {
  3: "basis-full sm:basis-[calc((100%-0.75rem)/2)] lg:basis-[calc((100%-1.5rem)/3)]",
  4: "basis-full sm:basis-[calc((100%-0.75rem)/2)] lg:basis-[calc((100%-2.25rem)/4)]",
};
...
export function GradeFerramentas({
  ...
  return (
    <div className="flex flex-wrap justify-center gap-3">
      {itens.map(({ href, titulo, Icone, descricao, chave, foto, imagemLocal, selo }) => (
        <Link
          key={href}
          href={href}
          className={cn(
            "group flex min-w-0 grow-0 flex-col gap-3 rounded-2xl border border-border bg-card/60 p-5 text-left transition-all hover:-translate-y-0.5 hover:border-primary/60 hover:shadow-[0_0_30px_-12px_var(--color-primary)]",
            LARGURA_CARTAO[colunas],
          )}
        >
          {/* O `relative` daqui é o que segura a etiqueta do `selo` no canto de
              cima da arte. Ele envolve os QUATRO jeitos de mostrar a mídia
              (imagem local, foto do serverrk, vídeo e o ícone grande) porque a
              etiqueta tem que cair no mesmo lugar em todos - inclusive no
              cartão sem arte, onde ela alinha com o ícone. */}
          <div className="relative">
            {selo && (
              <span className="absolute right-2 top-2 z-10 rounded-full border border-primary/50 bg-background/85 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-primary shadow-[0_0_18px_-6px_var(--color-primary)] backdrop-blur-sm">
                {selo}
              </span>
            )}
            {imagemLocal ? (
              <div className="aspect-square w-full overflow-hidden rounded-xl border border-border/60 bg-black/40">
                <img
                  src={imagemLocal}
                  alt=""
                  loading="lazy"
